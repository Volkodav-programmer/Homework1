...
def main():
    ...
    for i in range(count):
        ...
        animal.append(crit)
        animals_names.append(crit_name)
    animals=dict(zip(animals_names, animal))
    ...
    while choice != "0":
        ...
        5 - Покормить отдельную зверушку
        6 - Поиграть с отдельной зверюшкой
        """)
        ...
        # Купить зверюшку
            ...
            crit_name=input('Введите имя новой зверюшки(Имя не должно совпадать с уже существующими): ')
            crit = Critter(crit_name, random.randint(1, 10), random.randint(1,10))
            animal={crit_name: crit}
            animals.update(animal)

        # Покормить отдельную зверушку
        elif choice=='5':
            food=int(input('Количество еды: '))
            for i in animals_names:
                print("Имя зверушки:", i)
            animals[input('Введите имя зверушки, которую вы хотите покормить: ')].eat(food)
        # Поиграть с отдельной зверушкой
        elif choice=='6':
            fun=int(input('Количество минут: '))
            for i in animals_names:
                print("Имя зверушки:", i)
            animals[input('Введите имя зверушки, с которой вы хотите поиграть: ')].play(fun)
